...
export class AssetManager {
  ...
  async loadGLB(url,{clone=false}={}){
    if(!this.cache.has(url))this.cache.set(url,this.gltf.loadAsync(url));
    const gltf=await this.cache.get(url);
    if(!clone)return gltf;
    return {...gltf,scene:gltf.scene.clone(true)};
  }
  async loadJSON(url){
    if(!this.cache.has(url))this.cache.set(url,fetch(url).then(r=>{if(!r.ok)throw new Error(`${r.status} ${url}`);return r.json()}));
    return this.cache.get(url);
  }
  async loadTexture(url){
    if(this.cache.has(url))return this.cache.get(url);
    const p=/\.ktx2(?:\?|$)/i.test(url)?this.ktx2.loadAsync(url):new THREE.TextureLoader().loadAsync(url);
