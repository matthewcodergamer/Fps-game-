...
def reconstruct_package(manifest_path,manifest):
    root=manifest_path.parent; raw=root/'raw'; raw.mkdir(parents=True,exist_ok=True); source=raw/safe_name(manifest.get('sourceName') or 'source.glb','source.glb'); copy_parts(sorted((root/'chunks').glob('part-*.bin')),source,int(manifest.get('originalBytes') or 0))
    for dep in manifest.get('dependencies') or []:
        chunk_root=dep.get('chunkRoot')
        if not chunk_root: continue
        chunk_dir=ROOT/str(chunk_root); out=raw/'dependencies'/safe_name(dep.get('path') or chunk_dir.name,'dependency.bin'); copy_parts(sorted(chunk_dir.glob('part-*.bin')),out,int(dep.get('size') or 0))
    return source

def optimize_glb(path):
    opt=path.with_suffix('.optimized.glb'); r=run(['gltf-transform','optimize',str(path),str(opt),'--texture-compress','webp'],check=False)
    if r.returncode==0 and opt.exists() and opt.stat().st_size: opt.replace(path)
    else: opt.unlink(missing_ok=True); print('WARN: optimize failed; keeping Blender output',file=sys.stderr)
    if path.stat().st_size<TARGET_MAX: return
    for size in (1536,1024):
...
def validate_glb(path,report):
    ...
    if path.stat().st_size>=GITHUB_MAX: raise RuntimeError(f'Runtime GLB still exceeds GitHub 100 MiB limit: {path.stat().st_size} bytes')
    data=json.loads(report.read_text()) if report.exists() else {}; data['postOptimizationBytes']=path.stat().st_size; data.setdefault('assertions',{})['belowGitHub100MiB']=True; report.write_text(json.dumps(data,indent=2))
...
def convert_package(manifest_path):
    ...
    run(['blender','-b','-P',str(BLENDER_SCRIPT),'--',str(source),str(out),str(report)]); optimize_glb(out); validate_glb(out,report); print('VALIDATED',out.relative_to(ROOT),out.stat().st_size,'bytes'); return out
