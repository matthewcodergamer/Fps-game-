...
for(let attempt=0;attempt<15&&!download;attempt++){
  ...
  if(!download){
    ...
    for(const href of directHrefs){
      try{console.log('Trying href',href);const dPromise=page.waitForEvent('download',{timeout:20000});await page.goto(href,{timeout:30000});download=await dPromise;break;}catch{}
    }
  }
}
if(!download){
